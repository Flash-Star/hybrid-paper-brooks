...
while the white dwarf cools. To further investigate the efficacy
of these white dwarfs as progenitors of thermonuclear supernovae, we performed 
simulations of thermonuclear supernovae from a new series of hybrid progenitors
that include the effects of cooling. The progenitor white dwarf model
...
from previous models {\color{red} without the mixing during the white dwarf cooling.}
\end{abstract}
...
principally in the modules describing thermonuclear burning. 
specifically routines to describe
thermonuclear burning during both the deflagration and detonation
phases, as well as routines to describe the evolution of
the dynamic ash.

The need for a model flame in simulating thermonuclear supernovae
follows from the scales of the problem. At high densities, the
width of a laminar nuclear flame is $< 1\ensuremath{\;}{\ensuremath{\mathrm{cm}}}$
while the radius of the white dwarf 
$\sim 10^9\ensuremath{\;}{\ensuremath{\mathrm{cm}}}$.
Even with adaptive mesh refinement, whole-star simulations cannot simultaneousely
directly simulate the nuclear flame, thus requiring use of a model to
describe the burning on unresovable scales. For the deflagration phase,
our burning model propagates an artificially
broadened flame with an advection-diffusion-reaction (ADR)
...
these are boosted to account for speed-up of burning due to
unresolved buoyancy and background
turbulence~\cite{Khok95,gamezo.khokhlov.ea:thermonuclear,townsley.calder.ea:flame,jacketal2014}.
